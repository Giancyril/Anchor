...
import { LifeBuoy, AlertCircle, CheckCircle2, UserCheck, MessageSquare, Send, RefreshCw, Sparkles, Clock, ShieldAlert } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
          <span className="text-[11px] font-mono uppercase text-slate-400">Escalated Tickets</span>
          <p className="mt-1 text-2xl font-bold text-slate-900">{tickets.length}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
          <span className="text-[11px] font-mono uppercase text-slate-400">Live Waiting Queue</span>
          <p className="mt-1 text-2xl font-bold text-indigo-600">{queue.length}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
          <span className="text-[11px] font-mono uppercase text-slate-400">Webhook Status</span>
          <p className="mt-1 text-2xl font-bold text-emerald-600 font-mono">HMAC Verified</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[11px] font-bold text-slate-900">{t.ticket_id}</span>
                    ...
                    <p className="mt-1.5 text-xs font-medium text-slate-800 line-clamp-1">{t.title}</p>

                    <div className="mt-2 flex items-center justify-between text-[10px] font-mono text-slate-500">
                      <span>{t.intent}</span>
                      <span className="capitalize text-emerald-600 font-semibold">{t.status}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {selectedTicket && (
                    <span className="rounded-full bg-indigo-50 border border-indigo-200 px-2 py-0.5 text-[10px] font-mono text-indigo-700 font-semibold">
                ...
                <div className="space-y-2">
                  {cannedReplies.map((c, i) => (
                    <div
                      ...
                      className="cursor-pointer rounded-xl border border-slate-200 bg-slate-50/50 p-2.5 text-xs text-slate-700 hover:border-indigo-300 hover:bg-indigo-50/30 transition"
                    >
                      "{c}"
                    </div>
                  ))}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Human Agent Response & Notes</label>
              <textarea
                rows={4}
                value={replyText}
                onChange={(e) => setReplyText(e.target.value)}
                placeholder="Type your response to the customer or click an AI-suggested draft above..."
                className="w-full rounded-xl border border-slate-200 p-3 text-xs text-slate-900 focus:border-indigo-500 focus:outline-hidden"
              />
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-end gap-2 pt-3 border-t border-slate-100">
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-semibold text-white shadow-brand hover:bg-indigo-500 transition disabled:opacity-50"
            >
