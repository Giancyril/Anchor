  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/v1/search/hybrid", {
        ...
        body: JSON.stringify({ query, alpha, top_k: 4, enable_rerank: enableRerank, decompose }),
      });
      if (res.ok) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white sm:text-2xl">Hybrid Retrieval Inspector</h1>
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-900 pl-9 pr-4 py-2.5 text-xs text-white placeholder:text-slate-500 focus:border-indigo-500 focus:outline-hidden transition"
              />
            </div>
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 px-4 py-2.5 text-xs font-semibold text-white hover:from-indigo-500 hover:to-indigo-600 transition disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {results.map((res, idx) => (
          <div key={idx} className="rounded-2xl border border-slate-800/80 bg-[#111827] p-5 ring-1 ring-white/5 flex flex-col justify-between">
            <div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between text-[10px] font-mono text-slate-400">
              {res.breakdown.dense_rank !== undefined && <span>Dense: #{res.breakdown.dense_rank}</span>}
              {res.breakdown.sparse_rank !== undefined && <span>Sparse: #{res.breakdown.sparse_rank}</span>}
              {res.breakdown.rrf_score !== undefined && <span>RRF: {res.breakdown.rrf_score.toFixed(4)}</span>}
